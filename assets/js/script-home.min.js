...
var head=document.head||document.getElementsByTagName('head')[0];var style=document.createElement('style');style.type='text/css';head.appendChild(style);if(style.styleSheet){style.styleSheet.cssText=css}else{style.appendChild(document.createTextNode(css))}};injectCSS(styles);return sweetAlert$1})));if(typeof window!=='undefined'&&window.Sweetalert2)window.sweetAlert=window.swal=window.Sweetalert2;$(document).ready(function(){const path=$(location).attr("href");const file='wp-content/themes/olimar/source/Support/Sender.php';$('#form').submit(function(e){e.preventDefault();$(".contact__form__status").removeClass("d-none");$.ajax({url:path.concat(file),type:'POST',cache:!1,data:$('#form').serialize(),success:function(data){$('.contact__form__status').append(data);setTimeout(function(){$(".contact__form__status").addClass("d-none")},3000)},error:function(){$('.contact__form__status').append('Erro')}});return!1})})